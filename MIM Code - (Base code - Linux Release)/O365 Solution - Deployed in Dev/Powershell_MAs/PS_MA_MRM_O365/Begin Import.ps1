...
$MsolObjects = @(Get-ADUser -filter {msExchRecipientTypeDetails -eq 2147483648} -searchbase "OU=Standard Accounts,OU=Domain Accounts,DC=rfd,DC=lilly,DC=com" -searchscope 1  -Properties extensionAttribute4 |  Select-Object @{Name='Type';Expression={'User'}},saMAccountName,msExchRecipientTypeDetails, extensionAttribute4)
