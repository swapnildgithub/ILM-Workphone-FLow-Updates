...
namespace CommonLayer_NameSpace
{
    public static class CommonLayer
    {
        ...
        static string linuxGroupPath = string.Empty; 
        ...
        static CommonLayer()
        {
            ...
            //Get RF server for Linux LDAP query search
            node = rnode.SelectSingleNode("RFDC");
            strRFServer = node.InnerText;

            //Get RF server for Linux LDAP query search
            node = rnode.SelectSingleNode("linuxGroupPath");
            linuxGroupPath = node.InnerText;
        ...
        /// <summary>
        /// This function gets the membership of the group present in Resource forest)
        /// </summary>
        /// <param name="strSystemID"></param>
        /// <returns></returns>
        public static bool IsLinuxMember(MVEntry mventry)
        {
            ...
            DirectoryEntry linxGrpDirectoryEntry = new DirectoryEntry("LDAP://" + strRFServer + ":389/" + linuxGroupPath, null, null, AuthenticationTypes.Secure);
            PropertyCollection memberCollection = linxGrpDirectoryEntry.Properties;
            DirectorySearcher dSearcher = new DirectorySearcher(linxGrpDirectoryEntry);
            do
                {
                    ...
                    dSearcher.PropertiesToLoad.Add(attributeWithRange);
                    SearchResult results = dSearcher.FindOne();

                    if (results.Properties.Contains(attributeWithRange))
                    {
                        foreach (object member in results.Properties[attributeWithRange])
                        {

                            if (member.ToString().ToLower().Contains(mventry["sAMAccountName"].Value.ToLower()))
                            {
                                linxGrpDirectoryEntry.Close();
                                return true;
                            }
                        }
                        if (lastQuery)
        ...
        /// <param name="csObject"></param>
        /// <returns></returns>
        public static string GetUniqueMailNickname(string strdefaultmNickname, string strSamAccountName)
        {
